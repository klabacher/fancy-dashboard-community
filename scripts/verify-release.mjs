...
if (!SEMVER.test(String(pkg.version ?? "")))
  fail(`Invalid community SemVer: ${pkg.version}`);
if (compatibility.schemaVersion !== 1)
  fail("Unsupported compatibility schemaVersion");
if (compatibility.communityVersion !== pkg.version) {
  fail(
    `package.json version ${pkg.version} does not match compatibility.json communityVersion ${compatibility.communityVersion}`,
  );
}
if (!SEMVER.test(String(compatibility.core?.version ?? "")))
  fail("Invalid core version in compatibility.json");
...
if (compatibility.core?.repository !== "klabacher/FancyDashboard")
  fail("Unexpected core repository");
if (
  compatibility.toolchain?.node !== "24" ||
  compatibility.toolchain?.pnpm !== "9.12.3"
) {
  fail(
    "Community toolchain must stay aligned with core release toolchain (Node 24 / pnpm 9.12.3)",
  );
}
if (pkg.packageManager !== "pnpm@9.12.3")
  fail(
    `Root packageManager must be pnpm@9.12.3, received ${pkg.packageManager}`,
  );
...
for (const packDir of packDirs) {
  ...
  if (!SEMVER.test(String(pack.version ?? "")))
    fail(`Invalid pack version in ${packJsonPath}`);
  if (!pack.scripts?.build)
    fail(`Pack is missing build script: ${packJsonPath}`);
  if (
    pack.exports?.["."]?.import !== "./dist/index.js" ||
    pack.exports?.["."]?.require !== "./dist/index.cjs"
  ) {
    fail(`Pack must export ESM and CJS entrypoints: ${packJsonPath}`);
  }
  try {
    await fs.access(entryPath);
  } catch {
    fail(`Pack source entrypoint is missing: ${entryPath}`);
  }
  const manifest = await fs.readFile(manifestPath, "utf8");
  const versionMatch = manifest.match(/\bversion\s*:\s*["'`]([^"'`]+)["'`]/);
  if (!versionMatch) fail(`Missing manifest version in ${manifestPath}`);
  if (versionMatch[1] !== pack.version) {
    fail(
      `Package version ${pack.version} does not match manifest version ${versionMatch[1]} in ${manifestPath}`,
    );
  }
  if (/0{64}/.test(manifest))
    fail(`Placeholder zero integrity is forbidden: ${manifestPath}`);
...
process.stdout.write(
  `Community release metadata verified: v${pkg.version}, ${packDirs.length} pack(s), core ${compatibility.core.tag}.\n`,
);
